...
import { API_URL_SECTIONS } from '../../services/apiConfig';
...
const EditSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchSection = async () => {
      try {
        ...
        if (!isNaN(sectionId)) {
          const response = await axios.get(`${API_URL_SECTIONS} /${sectionId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`${API_URL_SECTIONS}/${id}`, updatedSection);
